...
extension Markdown {
  ...
  var blocksMarkdown : String {
    return slackMarkdownString
  }
}

extension Markdown: BlocksPrimitive {
  
  enum MarkdownRenderingError: Swift.Error {
  ...
  public func render(in context: BlocksContext) throws {
    ...
    switch block {
      ...
      case .richText :
        context.log.warning("attempt to use Markdown in RichText: \(context)")
        return try Section { self }
                     .render(in: context)
  ...
  private func renderIntoInput(in context: BlocksContext) throws {
    guard case .input(let input) = context.currentBlock else {
      ...
      throw MarkdownRenderingError.internalInconsistency
    }
  ...
  private func renderIntoContext(in context: BlocksContext) throws {
    ...
    ctxBlock.elements.append(.text(
      .init(slackMarkdownString, type: .markdown(verbatim: false))
    ))
  ...
  private func renderIntoImageBlock(in context: BlocksContext) throws {
    ...
    context.log.warning("rendering Markdown into Image, use Text: \(context)")
